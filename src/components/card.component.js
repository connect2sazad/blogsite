...
export const Card = ({ imageURL, categoryName, title, content, link, noOfViews, noOfComments }) => {
    return (
        <div class="p-4 md:w-1/3">
            <div class="h-full border-2 border-gray-200 border-opacity-60 rounded-lg overflow-hidden">
                <img class="lg:h-48 md:h-36 w-full object-cover object-center" src={imageURL} alt="blog" />
                <div class="p-6">
                    <h2 class="tracking-widest text-xs title-font font-medium text-gray-400 mb-1">{categoryName}</h2>
                    <h1 class="title-font text-lg font-medium text-gray-900 mb-3">{title}</h1>
                    <p class="leading-relaxed mb-3">{content}</p>
                    <div class="flex items-center flex-wrap ">
                        <Link to={link} class="text-indigo-500 inline-flex items-center md:mb-2 lg:mb-0">Learn More
